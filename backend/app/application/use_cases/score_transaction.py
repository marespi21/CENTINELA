...
class ScoreTransactionUseCase:
    ...
    sumar puntos → persistir score + detalle → si supera el umbral, generar la
    explicación legible y publicar el caso con ella. No se invoca desde la API:
    su disparador es el evento de la cola.
    """
    ...
    def __init__(
        ...
        self._explainer = explainer or RuleBasedExplainer()
    ...
    def execute(self, transaction: Transaction) -> ScoringResult:
        ...
        # adjuntando su explicación legible.
        if result.is_case:
            explanation = self._explainer.explain(result)
            self._cases.publish_case(result, explanation)
